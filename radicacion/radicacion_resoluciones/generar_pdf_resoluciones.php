...
/* Inicio funcion para definir cantidad de folios de un PDF */
...
/* Fin funcion para definir cantidad de folios de un PDF */
...
if($max2==0){
	...
	for ($q=0; $q < $max2; $q++){  // Recorre uno a uno los usuarios del listado separado por comas.
		...
		if($login_firmante==$login_elabora){
			...
			if($tipo_solicitud=="electronico" and $login_elabora==$login_usuario_busq){
...
if($numero_radicado==""){ // En este caso es un radicado nuevo 
	...
	$usuarios_visor 	= $lista_firma_aprueba_revisa."$login_elabora,";
	$usuarios_control  	= "$login_elabora,";

	/* Se arma la query para expediente cuando es un radicado nuevo */
	...
	/* Fin arma la query para expediente cuando es un radicado nuevo */
	...
	$numero_folios 	= numeroPaginasPdf("../../bodega_pdf/plantilla_generada_tmp/$aleatorio_recibido.pdf");	
	...
	$query_radicado="insert into radicado(numero_radicado, fecha_radicado, codigo_carpeta1, numero_guia_oficio, path_radicado, dependencia_actual, usuarios_visor, dependencia_radicador, usuario_radicador, asunto, leido, clasificacion_radicado, termino, id_expediente, codigo_serie, codigo_subserie, usuarios_control, estado_radicado, folios, medio_recepcion)values('$radicado', '$hora', $codigo_carpeta1, 'No se ha enviado al destinatario todavía, por lo tanto no hay soporte de envío.', '$path_pdf', '$dependencia_usuario', '$usuarios_visor', '$dependencia_usuario', '$login_usuario', '$asunto_doc', '$usuarios_visor', 'OFICIO', '15', '$id_expediente,', '$codigo_serie', '$codigo_subserie', '$usuarios_control','$estado_radicado','$numero_folios','radicacion_plantilla_general');";		

	/* Query para actualizar la version. Como es un radicado nuevo, se deja vacío */
	$query_version_documento2 = "";

	/* Variables para historico */
	$transaccion_historico 	= "Genera plantilla resoluciones";
	$transaccion 			= "plantilla_resoluciones"; 		// Variable para auditoria
	...
	if($id_expediente!=""){
		...
		$transaccion 			.= "_expediente"; 	// Variable para auditoria quedaría plantilla_resoluciones_expediente ó modifica_plantilla_resoluciones_expediente
	}
...
}else{	// En este caso es un radicado existente es decir, para modificar el radicado
	...
	$login_usuario 			= $login;
	$query_radicado = "update radicado set codigo_carpeta1=$codigo_carpeta1, path_radicado='$path_pdf', dependencia_actual='$dependencia_usuario', usuarios_visor='$usuarios_visor', dependencia_radicador='$dependencia_usuario', usuario_radicador='$login_usuario', asunto='$asunto_doc', leido='$usuarios_visor', id_expediente='$id_expediente', codigo_serie='$codigo_serie', codigo_subserie='$codigo_subserie' where numero_radicado='$numero_radicado';";

	// $query_radicado="insert into radicado(numero_radicado, fecha_radicado, codigo_carpeta1, numero_guia_oficio, path_radicado, dependencia_actual, usuarios_visor, dependencia_radicador, usuario_radicador, asunto, leido, clasificacion_radicado, termino, id_expediente, codigo_serie, codigo_subserie, usuarios_control) values('$radicado', '$hora', $codigo_carpeta1, 'No se ha enviado al destinatario todavía, por lo tanto no hay soporte de envío.', '$path_pdf', '$dependencia_usuario', '$usuarios_visor', '$dependencia_usuario', '$login_usuario', '$asunto_doc', '$usuarios_visor', 'OFICIO', '15', '$id_expediente,', '$codigo_serie', '$codigo_subserie', '$usuarios_control');";		
	...
	$transaccion_historico 	= "Modifica plantilla resoluciones";
	$transaccion 			= "modifica_plantilla_resoluciones"; 	// Variable para auditoria
...
if($tipo_solicitud=="electronico"){ // Si este documento es con firma electronica
    ...
    $aprobado = 'SI'; // Se define la variable para que no aparezca pendiente por aprobar ya que es impreso en físico
}

$query_version_documento = "insert into version_documentos(numero_radicado, version, usuario_modifica, fecha_modifica, path_pdf, html_asunto, usuario_que_firma, cargo_usuario_que_firma, firmado, usuario_que_aprueba, cargo_usuario_que_aprueba, aprobado, usuario_que_elabora, cargo_usuario_que_elabora, usuarios_revisa_aprueba, tamano, medio_solicitud_firmas)values('$radicado', '$version', '$login_usuario', '$fecha', '$path_pdf', '$pre_asunto', '$firmante_doc', '$cargo_firmante_doc', '$firmado', '$aprueba_doc', '$cargo_aprueba_doc', '$aprobado', '$elabora_doc', '$cargo_elabora_doc', '$listado_usuarios_revisa_aprueba', '$tamano', '$tipo_solicitud');$query_version_documento2";
	
// $query_radicado_dor_doc = $query_radicado.$query_version_documento;
$query_radicado_completo = $query_expedientes.$query_radicado.$query_version_documento.$inserta_ubicacion_fisica;
...
/* Inicio genera imagen_firma */
    ...
    if($tipo_solicitud=="electronico"){ // Si este documento es con firma electronica
        ...
        $imagen_firma = "
            <table border='0' style='font-size:12px; width: 650px;'>
                <tr>
               	...
               	<tr>
               		<td style='font-size:8px; padding_top:10px; width:100px;'>
               			<font style='font-weight:bold;'>Elaborado por: </font>$elabora_doc ($cargo_elabora_doc) $aprueba_doc_a $revisa_doc1_a $revisa_doc2_a $revisa_doc3_a $revisa_doc4_a $revisa_doc5_a
               		</td>
               	</tr>
            </table>";
...
/* Estas variables entre asteriscos deben definirse en template_final_resoluciones */
